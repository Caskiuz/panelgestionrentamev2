...
function CobroGestSidebar() {
  ...
  return (
    <>
      ...
      {open && <Overlay />}
      <aside
        className={`fixed md:static top-0 left-0 z-50 md:z-10 h-full md:h-auto w-64 bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 flex flex-col justify-between shadow-lg rounded-r-xl transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}
        style={{ minHeight: '100vh' }}
        tabIndex={-1}
        aria-label="Menú lateral"
      >
        <button
          className="md:hidden absolute top-4 right-4 text-white bg-gray-800 bg-opacity-80 p-2 rounded-lg z-50"
          onClick={() => setOpen(false)}
          aria-label="Cerrar menú"
        >
